...
thresholds_avg = thresholds_norm;
...
for k = 1:length(thresholds_norm)
    for j = 1:length(thresholds_avg)
        ...
        parfor t = 1:global_params.trials
            ...
            A_donoghue_occ_t = laplacian_to_adjacency(L_donoghue_occ);
            for a = 1:size(A_donoghue_occ_t,3) % threshold each interval's A
                A_tmp = A_donoghue_occ_t(:,:,a);
                A_tmp(A_tmp>donoghue_params.threshold) = 0;
                A_donoghue_occ_t(:,:,a) = A_tmp;
            end 
            A_donoghue_occ_avg = mean(A_donoghue_occ_t,3); % average thresholded A's
...
%% Outputs
save("thresholds_occ_n20_alt.mat");